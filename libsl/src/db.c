...
int sl_db_set(struct sldb *key, struct sldb *val)
{
	...
	mdata.mv_size = val->sz;

	err = mdb_txn_begin(env, NULL, 0, &txn);
	if (err) {
		warning("sl_db_set: mdb_txn_begin failed %m\n", err);
		return err;
	}

	err = mdb_put(txn, dbi, &mkey, &mdata, 0);
	if (err) {
		warning("sl_db_set: mdb_put failed %m\n", err);
		return err;
	}

	err = mdb_txn_commit(txn);
	if (err) {
		warning("sl_db_set: mdb_txn_commit failed %m\n", err);
		return err;
	}
...
int sl_db_init(void)
{
	...
	*/
	...
	if (err) {
		warning("sl_db_init: mdb_env_create failed %m\n", err);
		return err;
	}
	...
	if (err) {
		warning("sl_db_init: mdb_env_open failed %m\n", err);
		goto err;
	}

	err = mdb_txn_begin(env, NULL, 0, &txn);
	if (err) {
		warning("sl_db_init: mdb_txn_begin failed %m\n", err);
		goto err;
	}

	err = mdb_dbi_open(txn, NULL, 0, &dbi);
	if (err) {
		warning("sl_db_init: mdb_dbi_open failed %m\n", err);
		goto err;
	}

	err = mdb_txn_commit(txn);
	if (err) {
		warning("sl_db_init: mdb_txn_commit failed %m\n", err);
